...
export default function ImageAnnotations({doc, url}) {
    ...
    return <div className="relative h-96 w-96 mt-16">
        <div ref={parentDiv} className="object-scale-down">
            <img ref={image} onLoad={() => setCoords([image.current.width, image.current.height])} className="absolute" src={url} alt={doc.id} />
            {
            imageLoaded ?
                <svg className="absolute" viewBox={`0 0 ${x} ${y}`}>
                    ...
                    {/** From all annotations, we need to get  */}
                     {doc.data().object_detection.localizedObjectAnnotations.map((annotation, index) => {
                        ...
                        return <>
                        <text filter="url(#solid)" x={normalized[1].x} y={normalized[1].y} fill="blue" textAnchor="end" fontSize={x / 25}>{annotation.name}</text>
                        <path key={index} d={pathString(normalized)} fill="none" stroke="blue" stroke-width="5" />
                        </>
                     }
                      )}
                </svg>
